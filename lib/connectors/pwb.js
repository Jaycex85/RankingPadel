...
export async function searchPwb(nom, prenom) {
  ...
  try {
    ...
    // Quand aucun joueur ne correspond, PWB renvoie un tableau avec UNE entree
    // factice (ex: Nom = "Aucun resultat trouve") plutot qu'un tableau vide.
    // On filtre sur la presence d'un NumFed valide plutot que sur le texte
    // exact du message, qui pourrait changer.
    players = players.filter((p) => p && p.NumFed);

    // PWB fait une recherche floue cote serveur (tokenisee) qui peut renvoyer
    // des homonymes de nom avec un prenom totalement different. On refiltre
    // ici precisement sur Nom/Prenom pour ne garder que les vrais matches.
    players = players.filter((p) => {
      const nomOk = nom ? looseIncludes(p.Nom || "", nom) : true;
      const prenomOk = prenom ? looseIncludes(p.Prenom || "", prenom) : true;
