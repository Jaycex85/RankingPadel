...
export async function searchAfp(nom, prenom) {
  ...
  try {
    ...
    if (prenom) {
      // Filtre strict : si personne ne correspond au prenom, on renvoie une
      // liste vide plutot que de reafficher tous les homonymes de nom (ca a
      // deja cause un faux positif : un joueur absent d'AFP affichait quand
      // meme tous les autres "Moreno" par exemple).
      results = results.filter((r) => looseIncludes(r.nom_complet, prenom));
    }
